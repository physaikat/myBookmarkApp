...
function BookmarkAPI(){
    ...
    this.addremovebookmark = function(Obj, callback){
        var DBObj = Obj;
        this.getConnection(DBObj, function(err, conn){
            if(err)
            {
                ...
                var updatequery = {};
                if(DBObj.IsRemove){
                    updatequery = {$pull:{"bookmarkDetails":{"name":DBObj.bookmarkname}}}
                }
                else{
                    updatequery = {$push:{"bookmarkDetails":{"name":DBObj.bookmarkname,"url":DBObj.url}}}
                }
                conn.collection(DBObj.collectionName)
                    .update({'foldername':DBObj.foldername},updatequery,function(err, data){
                        disconnect(conn);
                        if (err) {
                            console.log("updateDataError. Cause: " + JSON.stringify(err));
                            callback(err, null);
